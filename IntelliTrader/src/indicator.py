...
class Indicator:
    ...
    def execute_handler(self, indicator_option, dataset):
        match indicator_option:
            ...
            case 'williams_range':
                self.option_williams_range(dataset)
            case _:
    ...
    def option_williams_range(self, dataset):
            try:
                if dataset is not None and not dataset.empty:
                    # Calculate Williams Range
                    pdf = pd.DataFrame(dataset)
                    williams_r_line = williams_r(pdf)
                    last_williams_range_value = williams_r_line[-1]

                    # Print the calculated Williams Range values
                    print("\nWilliams R Line:")
